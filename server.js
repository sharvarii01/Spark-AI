...
app.post('/api/chat', async (req, res) => {
  try {
    const { history, message, fileData } = req.body;
    ...
    const model = genAI.getGenerativeModel({ 
      ...
      systemInstruction: "You are Spark AI, a next-generation AI assistant. You were developed by Sharvari. Provide detailed, helpful, and comprehensive answers. Expand on key points to give the user a clear understanding of the topic."
    });
    ...
    while (attempt < maxRetries) {
      try {
        const result = await chatSession.sendMessage(msgParts.length === 1 && typeof msgParts[0] === 'string' ? msgParts[0] : msgParts);
